...
function getPermissionWord(user){
    let perm_list = []
    if(user.secure){
        perm_list.push('FWL_PERM_SEC_MASK')
    }
    if(user.nsecure){
        perm_list.push('FWL_PERM_NSEC_MASK')
    }
    if(user.priv){
        perm_list.push('FWL_PERM_PRIV_MASK')
    }
    if(user.user){
        perm_list.push('FWL_PERM_USER_MASK')
    }
    return perm_list;
}
    ...
    config: [
        ...
        },
        {
            ...
            },
            {
                ...
                description  : "Give Bin value of custom permission",
                hidden       : true,
                default      : 0,
