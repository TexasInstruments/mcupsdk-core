...
\note This step needs to be done once and is needed for the
  ...
  you need to edit the file `${SDK_INSTALL_PATH}/tools/ccs_load/am263px/load_sbl.js` and specify the SDK path in the file itself.
...
- Add a new variable named `MCU_PLUS_SDK_AM263PX_PATH` and point it to the path where the SDK is installed
...
- In Linux, usually you need to add a line as below in the ${HOME}/.bashrc,
        ...
        export MCU_PLUS_SDK_AM243X_PATH=${HOME}/ti/mcu_plus_sdk_am263px_{sdk version}/
...
###### AM263PX-LP
\attention This step needs to be done **every time** the AM263PX-LP is power-cycled.

- **POWER-OFF** the AM263PX-LP

- Make sure below cables are connected as shown in \ref EVM_CABLES
  - Power cable
  - JTAG cable

- Set AM263PX-LP in DEVBOOT mode as shown below

  \imageStyle{am263px_lp_boot_pins_noboot_mode.png,width:30%}
  \image html am263px_lp_boot_pins_noboot_mode.png "DEVBOOT MODE"

- **POWER-ON** the AM263PX-LP
...
- Type the below command in the scripting console and press "enter", to load DMSC FW and initialize the SOC
  - In Windows, assuming the SDK is installed at `C:/ti/mcu_plus_sdk_{soc}_{sdk version}`
        ...
        loadJSFile "C:/ti/mcu_plus_sdk_{soc}_{sdk version}/tools/ccs_load/am263px/load_sbl.js"

    \imageStyle{ccs_load_dmsc_00.png,width:50%}
    \image html ccs_load_dmsc_00.png "Scripting Console"

- In Linux, run the same command, only the path would be a Linux path like `/home/{username}/ti/mcu_plus_sdk_{soc}_{sdk version}/tools/ccs_load/am263px/load_sbl.js`
...
- If the script is run without providing power to the AM263PX-LP or if the AM263PX-LP BOOTMODE is
  ...
  - **SOLUTION**: Power cycle AM263PX-LP and repeat the steps.
...
- Set AM263PX-CC in DEVBOOT mode as shown below

  \imageStyle{boot_pins_noboot_mode.jpg,width:30%}
  \image html boot_pins_noboot_mode.jpg "DEVBOOT MODE"
...
- If the script is run without providing power to the AM263PX-CC or if the AM263PX-CC BOOTMODE is
  not set to \ref BOOTMODE_NOBOOT then you will see errors in the console and/or unexpected behaviour and error messages.
  - **SOLUTION**: Power cycle AM263PX-CC and repeat the steps.
