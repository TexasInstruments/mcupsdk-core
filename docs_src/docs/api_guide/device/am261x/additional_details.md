...
Below method cab be used for running examples built with makefiles
 ...
 - **Step 2:** Load and run the "hello world" example on the EVM, see \subpage CCS_LAUNCH_PAGE and \subpage IAR_LAUNCH_PAGE
...
- Set boot mode to UART BOOTMODE as shown in below image
  ...
  \imageStyle{am261x_lp_boot_pins_uart_mode.png,width:25%}
...
- Switch the EVM boot mode to OSPI mode as shown below,
  ...
  \imageStyle{am261x_lp_boot_pins_ospi.png,width:25%}
...
- Now you can run the example built (see \ref CCS_LAUNCH_PAGE \ref IAR_LAUNCH_PAGE)

- Connect to any of the Cortex_R5_0/Cortex_R5_1 cores, load programs and run (see \ref CCS_LOAD_RUN, \ref IAR_LOAD_RUN)
