...
- **Following services are supported in MCU+SDK v9.1.0**
    ...
    - **Write SW Revision Service** -: For HS-SE device only. write the software revision.
    - **Get KEK Service** -: For HS-SE device only. Get Derived KEK.
...
## HSM Client Get Kek Service. {#DRIVERS_HSMCLIENT_GET_KEK}
...
## HSM Client Get Random Number Service. {#DRIVERS_HSMCLIENT_GET_RAND_NUMBER}

- This service is available on both **HS_FS** and **HS-SE** devices.
- The service issued to HSM Server Returns the Random Number based on the instantiation of RNGReq_t object.
- User needs to instantiate RNGReq_t object and call HsmClient_getRandomNum() API to get random number.

## HSM Client KeyWriter. {#DRIVERS_HSMCLIENT_KEYWRITER}
