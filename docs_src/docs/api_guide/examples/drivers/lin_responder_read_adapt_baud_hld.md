# LIN Responder Read Adapt Baud Rate HLD {#EXAMPLES_DRIVERS_LIN_RESPONDER_READ_ADAPT_BAUD_HLD}

[TOC]

# Introduction

This example demonstrates LIN Responder mode data read operation with adaptive baudrate enabled where the Instance LIN1 is set as a Responder. The transfer is initiated by PLIN-USB (from PEAK Systems -> IPEH-004052) device connected to external PC.
...
# Supported Combinations {#EXAMPLES_DRIVERS_LIN_RESPONDER_READ_ADAPT_BAUD_HLD_COMBOS}
...
\cond SOC_AM263X || SOC_AM263PX
 ...
 Example folder | examples/drivers/lin/lin_responder_read_adapt_baud
...
- Click on LIN in the menu bar and connect to PLIN-USB. Ensure the LIN mode for PLIN is set to Commander with a baud rate that varies within a range of +/-2% from 19200.
