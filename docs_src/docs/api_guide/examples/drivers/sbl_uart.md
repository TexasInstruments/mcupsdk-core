...
This bootloader does SOC initializations and attempts to boot a multicore application image received over UART via XMODEM. The image file is sent using a python script (See \ref UART_BOOTLOADER_PYTHON_SCRIPT). Once image is received, the SBL parses it. Each core is then initialized, application image is loaded, entry points are set and the core is released from reset. For more on bootflow/bootloaders, please refer \ref BOOTFLOW_GUIDE

\cond SOC_AM263X || SOC_AM263PX || SOC_AM261X

# SBL UART MULTICORE ELF {#EXAMPLES_DRIVERS_SBL_UART_MCELF}

For AM26xx devices, use the Multicore ELF project of SBL UART
...
\cond SOC_AM263X || SOC_AM263PX || SOC_AM261X
 ...
 Toolchain      | ti-arm-clang
 Boards         | @VAR_BOARD_NAME_LOWER, @VAR_LP_BOARD_NAME_LOWER
 Example folder | examples/drivers/boot/sbl_uart_multicore_elf
...
Since this SBL receives the application image over UART using XMODEM protocol, it doesn't print anything to the console so as not to corrupt the XMODEM transport.