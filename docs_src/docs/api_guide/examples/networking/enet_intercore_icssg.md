...
This example shows how to use the shared memory-based Intercore driver for data traffic sharing across, coupled with the Ethernet driver (ENET) and LwIP bridge.
...
The example demonstrates the driver feature to exchange Ethernet (IEEE 802.1) packets between r5fss0-0 and r5fss0-1 cores, via shared memory and an Ethernet bridge (LwIP software bridge). 
CPU core r5fss0-0 fully owns & configures ICSSG1 peripheral resources and the corresponding DMA channels to handle packet reception/transmission directly from/to ICSSG1. It steers the traffic to core r5fss0-1 based on MAC address, using the LwIP bridge.
Separate MAC addresses are used to differentiate different CPU core traffic. Both the MAC addresses are added to ICSSG host port FDB entry so that HW accepts the traffic with those MAC addresses.
Separate TCP/IP (LiWP) stack instances are set up in each core, with an independent DHCP client, thus different IP addresses for each core. Furthermore, at application level, TCP echo servers are initialized on socket port 8888, in each core to handle the TCP traffic.
...
- CPU r5fss0-0 initializes ICSSG ethernet driver (LwIP netif 0) and the shared memory enet (LwIP netif 1) driver. Setups LwIP bridge with bridgeIF, linked to netif0 and netif1.
...
- Both r5fss0-0 and r5fss0-1 use RPMsg to exchange control messages, in addition to the shared memory interface (which is used to communicate packet data).
- TCP Server task running on each core awaits for connection from external client on port 8888. When connection is established, it waits for a message from the connected client.
- In response to external clients message. TCP Server task sends back "Greetings from Texas Instruments!" message back to the client and closes the connection.
...
Note: To configure any core other than r5fss0-0 as the main core, user needs to change the DMA channel resource allocation to the respective core, so that the ICSSG DMA channels for packet transfer are available to the configured main core. Please refer to  \ref RESOURCE_ALLOCATION_GUIDE to modify the resource allocation.
\endcond
...
-# This needs two unique channels open between a given pair of cores.
...
- If you see a valid, non-zero MAC address and continuously see "Waiting for network UP..." prints in UART terminal
