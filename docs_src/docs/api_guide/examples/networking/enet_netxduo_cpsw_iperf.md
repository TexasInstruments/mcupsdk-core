...
3. Open the NetxDuo Iperf web page in a browser by entering the target platform IP address.

4. To start the TCP or UDP transmit test, launch iperf on the host PC:
    \code
    $iperf -s -i 1 -p 5001
    \endcode

5. Click on the 'Start TCP Transmit Test' or 'Start UDP Transmit Test' button.

6. To start the TCP or UDP receive test, make sure your host PC IP address appears in the 'Destination IP Address' text box and click on the 'Start TCP Receive Test' or 'Start UDP Receive Test' button.

7. Launch iperf on the host PC, replacing 192.168.50.100 with the target actual IP address:
    \code
    $iperf -c 192.168.50.100 -i 1 -p 5001
    \endcode
