# Enet Lwip UDP Server Example{#EXAMPLES_ENET_LWIP_CPSW_UDPSERVER}
...
This example shows about how to implement a simple UDP Server on LwIP networking stack using netconn API coupled with ethernet driver (ENET)
...
- Initializes the LwIP stack for TCP/UDP IP and Starts UDP (echo) Server task.
- UDP Server task waits for any msg from client on port 8888 and echoes back the same message.
...
\cond SOC_AM64X
 ...
 Example folder | examples/networking/lwip/enet_cpsw_udpserver
...
Network is UP ...
     12. 71s : CPU load =   1.67 %
     17. 72s : CPU load =   1.73 %
     22. 73s : CPU load =   1.64 %
     27. 74s : CPU load =   1.64 %
     32. 75s : CPU load =   1.62 %
     37. 76s : CPU load =   1.58 %
     42. 77s : CPU load =   1.59 %
     47. 78s : CPU load =   1.62 %
\endcode
