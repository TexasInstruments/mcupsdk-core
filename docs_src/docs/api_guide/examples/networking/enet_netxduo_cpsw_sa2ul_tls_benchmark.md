# Enet NetxDuo CPSW and SA2UL TLS Benchmark Example{#EXAMPLES_ENET_NETXDUO_CPSW_SA2UL_TLS_BENCHMARK}
...
This example illustrates the configuration and usage of NetX secure using TLSv1.2 with RSA signed certificates, with and without crypto acceleration.
Alternatively, the example can be configured to use TLSv1.3 with ECDSA signed certificates. The example can be used to measure crypto acceleration performances. 
The CPSW controller is used in MAC mode (single port).

This example does the following
- Initializes the ethernet driver for the underlying HW.
- Initializes the NetxDuo stack for TCP/UDP IP.
- Initializes NetX secure with either a RSA (TLSv1.2) or ECDSA (TLSv1.3) self-signed server certificate.
- Receives HTTP request from client and sends 32MiB of data back as a response.  `curl` is used to measure the throughput.
- For TLSv1.2, crypto acceleration is used by default, but can be switched off for performance evaluation purposes.
...
The ECDSA self-signed certificate and private key are generated using
...
The RSA certificate and private key are generated using
...
Certificates are converted to DER using

\code
$ openssl x509 -outform der -in self_signed_cert.crt -out self_signed_cert.der
\endcode

Private keys are converted to DER using
...
Certificates and private keys are output as C arrays using
...
## Output as a private key C structure for SA2UL

For accelerated RSA, the RSA private key is converted to a SA2UL private key C structure using the provided script located in
`source/networking/netxduo/netxduo_enet/crypto_hw/tools`:

\code
python3 ./rsa_key_to_bigint.py rsa_private_key.key
\endcode
...
## Communicate with the EVM using curl

- You can send the initial HTTP request, download the test data and measure the throughput using `curl`. For TLSv1.3,
simply remove the `--tlsv1.2`, `--tls-max 1.2` and `--ciphers AES256-SHA256` options.

        > while true; do curl --tlsv1.2 --tls-max 1.2 --ciphers AES256-SHA256 -k \
        -w "time: %{time_total} | size: %{size_download} | response_code: %{response_code}\n" \
        -so /dev/null https://192.168.1.100; done
