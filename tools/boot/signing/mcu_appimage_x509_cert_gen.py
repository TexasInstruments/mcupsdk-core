...
1.3.6.1.4.1.294.1.35=ASN1:SEQUENCE:image_load
...
def get_cert(args):
	...
	if((args.bin is None) or (not os.path.exists(args.bin))):
		...
		if(args.enc == 'y'):
			if((args.enckey is None) or (not os.path.exists(args.enckey))):
				...
				enc_key = None
				with open(args.enckey, "rb") as f:
					enc_key = f.read()
				enc_key = binascii.hexlify(enc_key).decode('ascii')
	...
	ret_cert = g_x509_template.format(TEST_IMAGE_SHA_OID=v_TEST_IMAGE_SHA_OID,
                ...
                TEST_BOOT_ADDR=v_TEST_BOOT_ADDR,
