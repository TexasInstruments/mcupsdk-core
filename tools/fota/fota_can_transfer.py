...
BOOTLOADER_UNIFLASH_BUF_SIZE                         = 1024*4 # 1 MB This has to be a 256 KB aligned value, because flash writes will be block oriented
...
def send_number_of_files(totalFiles=0,gSer=None):
    '''
    ...
    file_ba = totalFiles.to_bytes(1,"little")
    status = WriteMessages(gSer, file_ba)
    time.sleep(0.005)
    in_data = str(ReadMessages(gSer)).replace("'","",2).replace("b","",1)
...
def send_receive_file(filename, get_response=True,gSer = None, total_parts=0,current_part=0):
    status = False
    timetaken = 0
    dataSize = 0
    try:
    ...
    while(total_parts == current_part):
        cmd="LSTCHNK"
        encoded=cmd.encode('ascii')
...
def send_file_by_parts(l_cfg,gSer = None):
    ...
    total_parts = num_parts + (remain_size>0)
    total_time_taken = 0
    ...
    for i in range(0, num_parts):
        ...
        status, timetaken = send_receive_file(tempfilename,True, gSer=gSer,total_parts=total_parts,current_part=i+1)
    ...
    if(remain_size > 0):
        ...
        # send the partial file normally
        tempfilename = create_temp_file(l_cfg)
        status, timetaken = send_receive_file(tempfilename,gSer=gSer,total_parts=total_parts,current_part=total_parts)
        total_time_taken += timetaken
        ...
        if(parse_status != 0):
            ...
            for i in range(0, len(filecfg.cfgs)):
                if(i != filecfg.flash_writer_index):
                    ...
                    status, timetaken = send_file_by_parts(linecfg, gSer = gSer)
